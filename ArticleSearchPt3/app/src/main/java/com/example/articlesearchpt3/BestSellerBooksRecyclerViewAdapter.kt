...
class BestSellerBooksRecyclerViewAdapter(
    ...
    private val listener: OnListFragmentInteractionListener? = null
    ...
    inner class BookViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        fun bind(book: BestSellerBook) {
            // Bind rank, title, author, and description
            rankingView.text = book.rank.toString()
            titleView.text = book.title
            authorView.text = book.author
            descriptionView.text = book.description

            // Load image with Glide, using placeholders and error handling
            ...
            imageView.contentDescription = "Cover image for ${book.title}"

            // Buy button click listener
            buyButton.setOnClickListener {
                book.amazonUrl.takeIf { it.isNotEmpty() }?.let { url ->
